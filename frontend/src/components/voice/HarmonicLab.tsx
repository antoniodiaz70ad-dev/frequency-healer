...
export default function HarmonicLab() {
  ...
  useEffect(() => {
    ...
    const stop = () => { invalidate(); engine.stop(); setPlaying(false); setBusy(false); };
    const hidden = () => { if (document.hidden) stop(); };
    document.addEventListener('visibilitychange', hidden); window.addEventListener('pagehide', stop);
    return () => { invalidate(); engine.dispose(); document.removeEventListener('visibilitychange', hidden); window.removeEventListener('pagehide', stop); };
  }, [engine]);
  ...
  return <div className={styles.workspace}><span className={styles.tag}>Relaciones exactas · motor aislado</span><h1>Laboratorio Armónico</h1><p>Configura una exploración sonora. Las relaciones matemáticas no demuestran efectos médicos.</p><section><h2>Diseño manual</h2><fieldset disabled={playing || busy}><div className={styles.grid}>
    ...
    {schedule && <section><h2>Propuesta visible</h2><SessionPlan config={config} schedule={schedule} /><p>Comienza con volumen cómodo. No conduzcas ni manejes maquinaria. Al ocultar la pestaña, el audio se detiene.</p>
      ...
      <button className={styles.primary} disabled={playing || busy || (config.ratioId === 'cascade-13-12' && !confirmed)} onClick={async () => { if (playing || startPending.current) return; startPending.current = true; const run = ++startRun.current; setBusy(true); setError(''); getAudioEngine().stopProtocol(); try { const started = await engine.start(config, () => setPlaying(false)); if (started && run === startRun.current) setPlaying(true); } catch (e) { if (run === startRun.current) setError((e as Error).message); } finally { if (run === startRun.current) { startPending.current = false; setBusy(false); } } }}>Confirmar e iniciar</button>
      {(playing || busy) && <button className={styles.stop} onClick={() => { ++startRun.current; startPending.current = false; engine.stop(); setPlaying(false); setBusy(false); }}>Detener sesión</button>}
