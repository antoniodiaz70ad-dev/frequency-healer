...
const ExperimentSession = forwardRef<ExperimentSessionHandle, { playbackActive: boolean }>(function ExperimentSession({ playbackActive }, ref) {
  ...
  const [record, setRecord] = useState<ExperimentRecordV1 | null>(null);
  const live = useRef<ExperimentRecordV1 | null>(null), savedRef = useRef(false), mounted = useRef(false), savingRef = useRef(false);
  ...
  useImperativeHandle(ref, () => ({
    ...
    started(id) {
      ...
      live.current = transitionExperiment(live.current, 'started'); setRecord(live.current);
    },
    finish(id, status, notify = true) {
      if (!id || live.current?.id !== id) return;
      live.current = transitionExperiment(live.current, status);
  ...
  const finalRecord = () => {
    ...
    return validateExperiment({ ...live.current, postState, ...(reflection.trim() ? { reflection } : {}) });
  };
  const reset = () => {
    live.current = null; savedRef.current = false; setRecord(null); setSaved(false); setDiscard(false); setEnabled(false);
  ...
  return <section aria-label="Registro opcional de experimentos"><h2>Registrar un experimento · opcional</h2>
    ...
    {record && <div className={styles.panel}>
      ...
      <details><summary>Configuración confirmada y datos previos</summary><pre>{JSON.stringify({ configurationSnapshot: record.configurationSnapshot, intention: record.intention, expectationScore: record.expectationScore, context: record.context, preState: record.preState }, null, 2)}</pre></details>
      ...
      {record.status !== 'started' && <>
        ...
        <button disabled={!editable} onClick={async () => {
          ...
          try {
            const value = finalRecord(); const rows = await new ExperimentStore(localStorage).save(value);
            live.current = value; savedRef.current = true;
            if (mounted.current) { setRecord(value); setHistory(rows); setSaved(true); }
        ...
        <button disabled={saving || playbackActive} onClick={() => { try { download(JSON.stringify(finalRecord(), null, 2), `experimento-${record.id}.json`); } catch (e) { setError((e as Error).message); } }}>Exportar este experimento</button>
