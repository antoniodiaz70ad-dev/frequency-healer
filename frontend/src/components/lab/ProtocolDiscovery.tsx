...
export default function ProtocolDiscovery({active,onConfirm,onStop}:{active:boolean;onConfirm(request:DiscoveryPlaybackRequest):Promise<void>;onStop():void}){
  ...
  const suggestedSeeds=intent?recommendGuided(intent).proposal.seedSelection?.candidates.filter(c=>c.valid).sort((a,b)=>b.structural.boundaryHeadroomOctaves-a.structural.boundaryHeadroomOctaves||a.structural.centerOffsetOctaves-b.structural.centerOffsetOctaves).slice(0,3).map(c=>c.seedHz)??[]:[];
  ...
  return <details><summary>Protocol Discovery · comparación personal</summary><section aria-label="Protocol Discovery"><h2>Protocol Discovery</h2>
    ...
    {draft&&<div><h3>Revisar plan antes de guardarlo</h3><p>{draft.candidates.length} candidatos · {draft.assignments.length} asignaciones · orden fijo.</p>{draft.candidates.map(c=><div key={c.id}><h4>{c.label} · {c.metadata.seedFrequencyHz} Hz</h4><SessionPlan config={c.config} schedule={buildSchedule(c.config)}/><ProtocolRationale value={c.rationale} config={c.config} constellation={c.constellation}/></div>)}<button disabled={blocked||!!record} onClick={()=>void run(async()=>{update(await new DiscoveryStore(localStorage).create(draft));setSelected(draft.id);setDraft(null);})}>Guardar plan Discovery</button></div>}
