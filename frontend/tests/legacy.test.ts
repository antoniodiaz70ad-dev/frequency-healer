...
class Param { value = 0; events:number[]=[]; setValueAtTime(v: number) { this.value = v;this.events.push(v); } cancelScheduledValues(){} linearRampToValueAtTime(v:number){this.value=v;this.events.push(v);} }
...
class Context {
  state = 'running'; currentTime = 0; sampleRate = 100; destination = { maxChannelCount: 2 }; oscs: Node[] = []; gains:Node[]=[]; pans: Node[] = []; noises: Node[] = [];
  resume = async () => {}; close = async () => { this.state = 'closed'; };
  createOscillator() { const n = new Node(); this.oscs.push(n); return n; }
  createGain() { const n=new Node();this.gains.push(n);return n; } createAnalyser() { return new Node(); }
...
test('atlas-style stop ramps gain to zero before oscillator shutdown',async()=>{
  const contexts:Context[]=[];Object.defineProperty(globalThis,'AudioContext',{configurable:true,value:class extends Context{constructor(){super();contexts.push(this);}}});
